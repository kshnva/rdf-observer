...
"""
Find pure energy of the lattice
"""
...
for i in range(len(mean_energy)):
	...
	formation.append(u)
	free_energy.append(get_free_energy(mean_energy[i], temperature[i]))

# for i in range(len(free_energy)):
# 	free_energy[i]=(free_energy[i]-(Au_conc[i]*free_energy[0])-((1-Au_conc[i])*free_energy[-1]))*96.484934

for energy, i in enumerate(free_energy):
	energy = energy - (Au_conc[i] * free_energy[0]) - ((1 - Au_conc[i]) * free_energy[-1])
	energy *= 96.484934
